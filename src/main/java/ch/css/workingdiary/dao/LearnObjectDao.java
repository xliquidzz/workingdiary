...
import ch.css.workingdiary.dao.mapper.LearnObjectMapper;
 ...
 */
@RegisterMapper(LearnObjectMapper.class)
public interface LearnObjectDao {

    @SqlQuery("SELECT * FROM learn_object where userId = :userId")
    List<LearnObject> getLearnObjects(@Bind("userId") final long userId);
}
